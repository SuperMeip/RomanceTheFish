...
public class MessageManager : MonoBehaviour {
    ...
    public void RetreiveDialogs()
    {
        ...
        Dialogs = new Message[rawMessages.Length-1];
        int index = 0;
        foreach (string message in rawMessages)
        {
            if (message != "")
            {
                Dialogs[index] = new Message(message);
                index++;
            }
        }
    ...
    public string PlayLine(Message dialog)
    {
        ...
        {
            CharacterSprite.sprite = CharacterInfo.instance.CharacterList[dialog.Character].Sprites[dialog.Lines[currentMessage].Emotion];
